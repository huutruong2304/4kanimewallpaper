...
export async function getImagesNew(page: number) {
  return fetcher<ApiResponse<ImageDetail[]>>(`/new?page=${page}`, {
    next: { revalidate: 60 * 60 },
  });
}

export async function getImagesHot(page: number) {
  return fetcher<ApiResponse<ImageDetail[]>>(`/hot?page=${page}`, {
    next: { revalidate: 60 * 60 },
  });
}

export async function getImagesPopular(page: number) {
  return fetcher<ApiResponse<ImageDetail[]>>(`/popular?page=${page}`, {
    next: { revalidate: 60 * 60 },
  });
}
